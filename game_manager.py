...
class GameManager:
	...
	def get_bids(self):
		...
		for i in range(4):
			...
			self.players[active_player_id].give_hand(deepcopy(self.hands[active_player_id]))
			self.bids[active_player_id] = self.players[active_player_id].make_bid(self.bids.copy())
	...
	def play_round(self):
		...
		if _BLIND_NILL_CHARACTER in self.bids.values():
		...
		for i in range(13):
			...
			for player in self.players:
				player.announce_trick(deepcopy(current_trick))
		# Handle the scoring
		for i in [0, 1]:
